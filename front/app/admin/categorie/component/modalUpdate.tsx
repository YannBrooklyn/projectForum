            ...
            <div id="modalEditer" className="flex flex-col justify-center items-center fixed h-96 w-96 lg:w-3/6 xl:w-2/6" style={{ zIndex:"3", backgroundColor:params.backgroundColorThird}}>
                <form  id="testForm" className="h-full w-full" onSubmit={handleUpdateCateg} action="" method="put">
                    <div className=" h-full w-full flex flex-col justify-center items-center gap-6">
                        ...
                        <select style={{color:params.textColor, backgroundColor:params.backgroundColorZoneText}} value={theme}  onChange={(b) => setTheme(b.target.value)} name="" id="">
                            <option  style={{color:params.textColor, backgroundColor:params.backgroundColorZoneText}} value="0">Selectionnez un thême</option>
                            {allTheme.map((theTheme, index)=>{

                                return (
                                    <option style={{color:params.textColor, backgroundColor:params.backgroundColorZoneText}} key={index} value={theTheme.id}>{theTheme.nameTheme}</option>
                                )
                            })}
                        </select>
